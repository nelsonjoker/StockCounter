...
namespace Barcodes {
   class Barcode39 {
      ...
      private string ExtendedString(string s) {
        ...
        public void MakeBarcodeImage(string InputData, Graphics gr, Rectangle area)
        {
            int ImageWidth = area.Width;
            int ImageHeight = area.Height;
            ...
            //gr.FillRectangle(Brushes.Red, offsetX, offsetY, ImageWidth, ImageHeight);
            ...
            ExtString = ExtendedString(InputData);
            ...
            int Q = 6;
            //int bcWidth = (EncodedString.Length + 2) * (3 * 2 + 6) * 1 + (EncodedString.Length + 1) * 1 + 2 * Q;
            int bcWidth = (EncodedString.Length + 1) * (3 * 2 + 6) * 1 +  2 * Q;
            double scale = ImageWidth / ((double)bcWidth);
            ...
            if (ShowString)
            {
                if ((TextFont != null))
                {
                    ...
                    float H = gr.MeasureString(InputData, TextFont).Height;
                    gr.DrawString(InputData, TextFont, textBrush, offsetX + 0, offsetY + ImageHeight - H);
            ...
            DrawBarcode(gr, EncodedString, ImageHeight, scale, offsetX + (int)(scale*Q), offsetY);
      ...
      public Image GenerateBarcodeImage(int ImageWidth, int ImageHeight, string OriginalString) {
        ...
        {
            DrawBarcode(g, EncodedString, Height, 1.0, 0, 0);
        }
